...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::unprepared('
            DROP PROCEDURE IF EXISTS update_complaint_status;

            CREATE PROCEDURE update_complaint_status(
                IN complaintID INT
            )
            BEGIN
                 UPDATE complaints_department 
                SET proses = "selesai"
        ...
        // DELETE complaint
        DB::unprepared('
            DROP PROCEDURE IF EXISTS delete_complaint;
    
            CREATE PROCEDURE delete_complaint(
                IN complaintID INT
            )
            BEGIN
                DELETE FROM complaints_department 
                WHERE id = complaintID;
            END
        ');

        // TRIGGER to complaint_history
        DB::unprepared("
            DROP TRIGGER IF EXISTS throw_complaint_history;

            CREATE TRIGGER throw_complaint_history
            BEFORE DELETE ON complaints_department
            FOR EACH ROW
                IF OLD.proses = 'selesai' THEN
                    INSERT INTO complaint_history (
                        id, user_id, category_id, attachment_id, department_id, response_id,
                        complaint_title, complaint_content, proses, created_at, updated_at
                    ) VALUES (
                        OLD.id, OLD.user_id, OLD.category_id, OLD.attachment_id, OLD.department_id,
                        OLD.response_id, OLD.complaint_title, OLD.complaint_content, OLD.proses,
                        OLD.created_at, OLD.updated_at
                    );
                END IF;
        ");

        // SELECT complaint_history => history
        DB::unprepared(<<<SQL
        DROP PROCEDURE IF EXISTS select_complaint_history;
    
        CREATE PROCEDURE select_complaint_history(
            IN search_keyword VARCHAR(255),
            IN filter_type VARCHAR(10),
            IN userID INT
        )
        ...
        // SELECT complaint_history detail
        DB::unprepared('
            DROP PROCEDURE IF EXISTS select_complaint_history_detail;

            CREATE PROCEDURE select_complaint_history_detail(
                IN complaintID INT
            )
            BEGIN
                SELECT 
                ...
                LEFT JOIN users u ON m.user_id = u.id
                LEFT JOIN complaint_attachment a ON c.attachment_id = a.id
                LEFT JOIN complaint_response r ON c.response_id = r.id
                WHERE c.id = complaintID;
            END
        ');
        DB::unprepared('
            DROP PROCEDURE IF EXISTS select_users_history;

            CREATE PROCEDURE select_users_history(
                IN complaintID INT
            )
            BEGIN
                SELECT 
                ...
                users.profile_picture
                FROM users
                INNER JOIN complaint_history ON users.id = complaint_history.user_id
                WHERE complaint_history.id = complaintID;
            END
        ');
    }
    ...
    public function down(): void
    {
        ...
        DB::unprepared('
            DROP TRIGGER IF EXISTS delete_complaint
        ');

        DB::unprepared('
            DROP TRIGGER IF EXISTS select_complaint_history
        ');

        DB::unprepared('
            DROP TRIGGER IF EXISTS select_complaint_history_detail
        ');

        DB::unprepared('
            DROP TRIGGER IF EXISTS select_users_history
        ');
    }
};
